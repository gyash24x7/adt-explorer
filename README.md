...
1. Setup a system assigned identity to allow Functions proxy to access Azure Digital Twins Service.
    1. In Azure, open the **Function Explorer** resource from your resource group.
    ...
    1. Assign accesss to **System assigned managed identity - Functions Explorer**.
    1. Select your **Functions Explorer** from the list.
    1. Click **Save**.
...
New plugins need to be registered in `ImportPlugins` collection at the top of the `src/services/ImportService.ts` file.
...
Export is managed by the `ExportService` class in `src/services/ExportService.ts` file.
...
All panels are defined in the `src/Explorer.tsx` file. These configuration objects are defined by the requirements of the Golden Layout Component.
...
All events - via publish and subscribe methods - are defined in the `src/services/EventService.ts` file. Additional events can be defined by adding to this file.
...
When running locally, all requests to the ADT service are proxied through the same local web server used for hosting the client app. This is configured in the `client/src/setupProxy.ts` file.
