...
export type GraphViewerTwinDeleteComponentProps = {
...
export class GraphViewerTwinDeleteComponent extends Component<GraphViewerTwinDeleteComponentProps> {
	...
	async deleteSelected() {
		...
		if (onDelete) {
			const ids =
				selectedNodes.length > 1 ? selectedNodes.map(node => node.id()) : [selectedNode!.id()];
			onDelete(ids);
		}
		...
		return (
			<ModalComponent isVisible={showModal} isLoading={isLoading} className="gc-dialog">
				...
				<div className="btn-group">
					<DefaultButton className="modal-button save-button" onClick={this.confirm}>
						Delete
					</DefaultButton>
