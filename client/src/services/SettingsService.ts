...
export type Settings = {
	...
	queries: any[];
	relTypeLoading: string;
	relExpansionLevel: number;
};

export type Environment = {
	name: string;
	config: { appAdtUrl: string };
};
...
class SettingsService {
	...
	get environments() {
		return storageService.getLocalStorageObject(EnvStorageKeyName) as Environment[];
	}

	set environments(envs: Environment[]) {
		storageService.setLocalStorageObject(EnvStorageKeyName, envs);
	}

	getModelImage = (modelId: string) => storageService.getLocalStoragePrimitive(modelId);

	setModelImage = (modelId: string, dataString: any) =>
		storageService.setLocalStorageObject(modelId, dataString);

	deleteModelImage = (modelId: string) => storageService.removeLocalStorageObject(modelId);
