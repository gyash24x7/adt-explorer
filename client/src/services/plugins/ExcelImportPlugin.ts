...
class StandardExcelImportFormat {
	getData(resp: any) {
		...
		for (let i = 1; i < resp.rows.length; i++) {
			...
			const twinId: string = row[headers[IdColumn]];
			if (!twinId) {
				throw new Error(`Missing twin ID on row ${i}`);
			}

			const modelId: string = row[headers[ModelColumn]];
	...
	_mapHeaders(row: any) {
		const mapping = ColumnMapping.reduce((p, c) => {
			const index = row.findIndex((x: any) => x === c.name);
			if (index >= 0) {
				p[c.id] = index;
			}
...
export class ExcelImportPlugin {
	async tryLoad(file: File): Promise<DataModel | undefined> {
